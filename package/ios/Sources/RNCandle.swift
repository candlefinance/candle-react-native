...
@available(iOS 17.0, *) final class HybridRNCandle: HybridRNCandleSpec {
    ...
    public func candleLinkSheet(
        ...
        Task { @MainActor in
            ...
            try viewModel.$linkedAccount.removeDuplicates().compactMap(\.?.toLinkedAccount)
    ...
    public func candleTradeExecutionSheet(
        ...
        Task { @MainActor in
            ...
            wrapperView.viewModel.$tradeResult.receive(on: RunLoop.main)
                .sink { result in
                    switch result {
                    case .success(let trade): completion(.init(trade: trade.toRNModel, error: nil))
    ...
    public func unlinkAccount(ref: LinkedAccountRef) throws -> Promise<Void> {
        .async {
            try await Client.shared.unlinkAccount(ref: .init(linkedAccountID: ref.linkedAccountID))
        }
    }
    ...
    public func getLinkedAccount(ref: LinkedAccountRef) throws -> Promise<LinkedAccount> {
        .async {
            let account = try await Client.shared.getLinkedAccount(
    ...
    {
        .async {
            let accounts = try await Client.shared.getAssetAccounts(
                query: .init(
                    linkedAccountIDs: query.linkedAccountIDs,
                    assetKind: query.assetKind?.toAssetKindPayload
                )
            )
    ...
    public func getAssetAccount(ref: AssetAccountRef) throws -> Promise<AssetAccount> {
        .async {
            let account = try await Client.shared.getAssetAccount(
                ref: .init(
    ...
    public func getTrade(ref: TradeRef) throws -> Promise<Trade> {
        .async {
            let trade = try await Client.shared.getTrade(
                ref: .init(
    ...
    public func getTrades(query: TradeQuery) throws -> Promise<TradesResponse> {
        .async {
            let trades = try await Client.shared.getTrades(
                query: .init(
                    linkedAccountIDs: query.linkedAccountIDs,
            ...
            return TradesResponse(
                linkedAccounts: trades.linkedAccounts.map(\.toRNModel),
    ...
    public func getTradeQuotes(request: TradeQuoteRequest) throws -> Promise<TradeQuotesResponse> {
        .async {
            ...
            return TradeQuotesResponse.init(
                linkedAccounts: accounts.linkedAccounts.map(\.toRNModel),
                tradeQuotes: accounts.tradeQuotes.map { account in
                    TradeQuote(
    ...
    var toRNModel: Models.TradeAssetQuoteRequest {
        get throws {
            if let fiatAssetQuoteRequest {
                ...
                else { throw RNClientError.badEncoding }
                ...
                return Models.TradeAssetQuoteRequest.TransportAssetQuoteRequest(
                    .init(
                        ...
                        originCoordinates: transportAssetQuoteRequest.originCoordinates?
                            .toCoordinates,
                        ...
                        destinationCoordinates: transportAssetQuoteRequest.destinationCoordinates?
                            .toCoordinates,
                        destinationAddress: transportAssetQuoteRequest.destinationAddress?
                            .toAddress,
...
extension Address { var toAddress: Models.Address { return .init(value: value) } }

extension Coordinates {
    var toCoordinates: Models.Coordinates { return .init(latitude: latitude, longitude: longitude) }
}
...
extension AssetAccountKind {
    var toAssetKindPayload: Models.GetAssetAccounts.Input.Query.AssetKindPayload {
        switch self {
...
extension TradeQuery {
    ...
    {
        if let gainedAssetKind { return .init(rawValue: gainedAssetKind) }
        return nil
    }

    var toLostAssetKind: Candle.Operations.GetLinkedAccountsTrades.Input.Query.LostAssetKindPayload?
    {
        if let lostAssetKind { return .init(rawValue: lostAssetKind) }
        return nil
    }
    ...
    {
        if let counterpartyKind { return .init(rawValue: counterpartyKind) }
        return nil
    }
}
